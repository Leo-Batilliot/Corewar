...
champ_t *init_champion(flags_t *flags, corewar_t *corewar)
{
    ...
    champion->color_index = corewar->nb_robot + 1;
    corewar->cur_id++;
...
static int init_cycles(corewar_t *main_struct)
{
    ...
    main_struct->sim->names = NULL;
    main_struct->sim->list = NULL;
    return 0;
}

static int init_loading(corewar_t *corewar)
{
    char file[32];

    corewar->textures = malloc(sizeof(sfTexture *) * 10);
    corewar->sprite = sfSprite_create();
    corewar->texture_index = 0;
    corewar->clock = sfClock_create();
    if (!corewar->sprite || !corewar->textures || !corewar->clock)
        return 1;
    for (int i = 0; i < 1; i++) {
        snprintf(file, 32, "assets/loading%i.png", i);
        corewar->textures[i] = sfTexture_createFromFile(file, NULL);
        if (!corewar->textures[i]) {
            corewar->textures = NULL;
            return 1;
        }
    }
    sfSprite_setTexture(corewar->sprite, corewar->textures[0], sfTrue);
    corewar->textures[1] = NULL;
    return 0;
...
corewar_t *init_main_struct(sfRenderWindow *window)
{
    ...
    main_struct->dump = -1;
    if (init_cycles(main_struct) || init_loading(main_struct)) {
        my_free(main_struct);
        return NULL;
    }
    return main_struct;
